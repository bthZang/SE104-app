...
import { PAYSLIP_API, REQUEST_API, TIMEKEEPING_API } from '../constant/apiURL';
...
export const getAllTimekeeping = async (accessToken) => {
    try {
        const res = await axios.get(`${TIMEKEEPING_API}/all`, {
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
        })
